...
public class SKUWiseFragmentThreeTab<Context> extends Fragment 
{
            ...
            Bundle savedInstanceState) 
    {
       ...
       // TelephonyManager tManager = (TelephonyManager) mContext.getSystemService(mContext.TELEPHONY_SERVICE);
		imei = AppUtils.getToken(mContext); //tManager.getDeviceId();
		...
		{
			//hmapProductTblUOMMapping=dbengine.fetch_ProductWiseGetAllMeasureUnitAgainstProductForLastVisitPage("0");
		...
		if(hmapAll_details.size()>0)
		{
	       ...
	       //  Go_Ch_Wed_PL_140gm^90^81.82^2^0^33.06^330.581^41.32^371.901^1^2
			...
			for(int i=0;i<AllDataContainer.length;i++)
			{
				...
				String s18 = tokens1.nextToken().toString().trim();
	            ...
	            LinearLayout ll_store_sku=(LinearLayout) viewParent.findViewById(R.id.ll_store_sku);
		         ...
		         else if(Integer.parseInt(s11)==2)
		         {
					...
					/*String pcsPart="0";
					String casePart="0";
					 if(hmapProductTblUOMMapping!=null && hmapProductTblUOMMapping.size()>0 && hmapProductTblUOMMapping.containsKey(Integer.parseInt(s18)))
					 {
						 Double otqy=Double.parseDouble(s4);
						 List<TblUOMMapping> tblUOMMappings= hmapProductTblUOMMapping.get(Integer.parseInt(s18));
					 ...
					 if((int)(Double.parseDouble(casePart))>0)
					 {
						 casepcstext=(int)(Double.parseDouble(casePart)) +" Cs";
					 }
					 if((int)(Double.parseDouble(pcsPart))>0)
					 {
					 	if(!casepcstext.equals("0"))
						{
							casepcstext = (int) (Double.parseDouble(pcsPart)) + " Pc";
						}
					 	else {
							casepcstext = casepcstext +" "+ (int) (Double.parseDouble(pcsPart)) + " Pc";
						}
					 }*/
