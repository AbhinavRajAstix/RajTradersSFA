	...
	private void intializeFields() 
	{
		...
		if(AllDataContainer.length>0)
		{
         ...
         String a15 = tokens.nextToken().toString().trim();
			String a16 = tokens.nextToken().toString().trim();
		...
		for(int i=1;i<AllDataContainer.length;i++)
		{
			...
			String s16 = tokens1.nextToken().toString().trim();

	         System.out.println("Value of level :"+s13);
	         
	         
	        LayoutInflater inflaterParent=(LayoutInflater) getSystemService(LAYOUT_INFLATER_SERVICE);
